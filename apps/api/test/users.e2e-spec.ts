...
describe("Users (e2e)", () => {
  ...
  afterAll(async () => {
    if (testUser && testUser.id) {
      ...
      await dataSource.query("DELETE FROM user_progress WHERE user_id = $1", [
        testUser.id,
      ]);
      await dataSource.query("DELETE FROM chat_sessions WHERE user_id = $1", [
        testUser.id,
      ]);
      await dataSource.query("DELETE FROM quiz_sessions WHERE user_id = $1", [
        testUser.id,
      ]);
      await dataSource.query("DELETE FROM xp_events WHERE user_id = $1", [
        testUser.id,
      ]);
      await dataSource.query("DELETE FROM user_badges WHERE user_id = $1", [
        testUser.id,
      ]);
      await dataSource.query("DELETE FROM certificates WHERE user_id = $1", [
        testUser.id,
      ]);
      await dataSource.query(
        "DELETE FROM gallery_projects WHERE user_id = $1",
        [testUser.id],
      );
