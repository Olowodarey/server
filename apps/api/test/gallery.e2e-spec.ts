...
describe("Gallery (e2e)", () => {
  ...
  afterAll(async () => {
    if (testUser && testUser.id) {
      ...
      await dataSource.query("DELETE FROM project_votes WHERE user_id = $1", [
        testUser.id,
      ]);
  ...
  describe("/api/v1/gallery (GET)", () => {
      ...
      return request(app.getHttpServer())
        ...
        .expect((res) => {
          if (res.statusCode !== 200)
            console.log("Gallery GET fail:", res.body.error?.message);
    ...
    it("should filter by category", () => {
      return request(app.getHttpServer())
        ...
        .expect((res) => {
          if (res.statusCode !== 200)
            console.log("Gallery Filter fail:", res.body.error?.message);
        })
        .expect(200);
    });
  });
